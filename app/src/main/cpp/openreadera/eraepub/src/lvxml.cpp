...
#define MAX_PRE_STATS 1000
        ...
        if (!smart_format_flags_ && pmlTagCount>20 ) {
                ...
                if(attrname == "class")
                {
                    ...
                    for (int i = 0; i < classes.length(); i++)
                    {
                        ...
                        if(stylesManager.classExists(name))
                        {
                            ...
                            img_source = CssClass.getBackground();
                            if(!img_source.empty())
                            {
...
bool LvXmlParser::ReadTextToString(lString16 & output, bool write_to_tree, bool rtl_force_check)
{
    ...
    while ( !flgBreak ) {
        ...
        if ( tlen > TEXT_SPLIT_SIZE || flgBreak || splitParas)
        {
            ...
            lString16 processed;
            if(DVNG_DISPLAY_ENABLE == 1)
            {
                if(gDocumentDvng == 0)
                {
                    lString16(buf, nlen).CheckDvng();
                }
                if(gDocumentDvng == 1)
                {
                    processed = lString16::processDvngText(lString16(buf, nlen));
                    buf = processed.modify();
                    nlen = processed.length();
                }
            }

            if(BANGLA_DISPLAY_ENABLE == 1)
            {
                if(gDocumentBangla == 0)
                {
                    lString16(buf, nlen).CheckBangla();
                }
                if(gDocumentBangla == 1)
                {
                    //LE("before = [%s]",LCSTR(lString16(buf,nlen)));
                    processed = lString16::processBanglaText(lString16(buf, nlen));
                    buf = processed.modify();
                    nlen = processed.length();
                    //LE("after = [%s]",LCSTR(lString16(buf,nlen)));
                }
            }
            ...
            if (flags & TXTFLG_PRE) {
                ...
                if ( tabCount > 0 ) {
                    ...
                    if(gDocumentDvng || gDocumentBangla)
                    {
                        output = processed;
                    }
                    else
