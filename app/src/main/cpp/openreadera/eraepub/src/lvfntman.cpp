...
public:
    ...
    virtual bool getGlyphInfoDvng(lUInt16 code, glyph_info_t * glyph, bool *valid)
    {
        ...
        LVFontRef dvngFont = fontMan->GetFont(this->getSize(), 900, this->getItalic(), css_ff_sans_serif, DEVANAGARI_FACE_DEFAULT);
        #else
        LVFontRef dvngFont = fontMan->GetFont(this->getSize(),this->getWeight(),this->getItalic(),css_ff_sans_serif,DEVANAGARI_FACE_DEFAULT);
        #endif
        if (dvngFont.isNull())
        {
            *valid = false;
        ...
        else if(lString16::CharIsDvng(code))
        {
            glyph_index = dvngFont->getCharIndex(code, 0);
        }
        if (glyph_index != 0)
        {
            *valid = true;
            return dvngFont->getGlyphInfoItem(glyph_index, glyph);
        }
        *valid = false;
        return false;
    }
    ...
    virtual bool getGlyphInfoBangla(lUInt16 code, glyph_info_t * glyph, bool *valid)
    {
        ...
        LVFontRef banglaFont = fontMan->GetFont(this->getSize(), 900, this->getItalic(), css_ff_sans_serif, BANGLA_FACE_DEFAULT);
        #else
        LVFontRef banglaFont = fontMan->GetFont(this->getSize(),this->getWeight(),this->getItalic(),css_ff_sans_serif,BANGLA_FACE_DEFAULT);
        #endif
        if (banglaFont.isNull())
        {
            *valid = false;
            return false;
        }
        ...
        else if(lString16::CharIsBangla(code))
        {
            glyph_index = banglaFont->getCharIndex(code, 0);
        }

        if (glyph_index != 0)
        {
            *valid = true;
            return banglaFont->getGlyphInfoItem(glyph_index, glyph);
        }
        *valid = false;
        return false;
    }
...
//        }
    ...
    virtual LVFontGlyphCacheItem* getGlyphDvng(int ch)
    {
        ...
        else if(lString16::CharIsDvng(ch))
        {
            ch_glyph_index = dvngFont->getCharIndex(ch, 0);
        }
        if (ch_glyph_index != 0)
        {
            return dvngFont->getGlyphItem(ch, ch_glyph_index);
        }
        return NULL;
    }
    ...
    virtual LVFontGlyphCacheItem* getGlyphBangla(int ch)
    {
        ...
        else if(lString16::CharIsBangla(ch))
        {
            ch_glyph_index = banglaFont->getCharIndex(ch, 0);
        }
        if (ch_glyph_index != 0)
        {
            return banglaFont->getGlyphItem(ch, ch_glyph_index);
        }
        return NULL;
    }
    ...
    */
    virtual LVFontGlyphCacheItem *getGlyph(lUInt16 ch, lChar16 def_char=0)
    {
        ...
        if(DVNG_DISPLAY_ENABLE == 1 && gDocumentDvng == 1 )
        {
            LVFontGlyphCacheItem* res = getGlyphDvng(ch);
            if(res != NULL)
            {
                return res;
            }
        ...
        if(BANGLA_DISPLAY_ENABLE == 1 && gDocumentBangla == 1 )
        {
            LVFontGlyphCacheItem* res = getGlyphBangla(ch);
            if(res != NULL)
            {
                return res;
            }
...
public:
    ...
    bool checkFace(FT_Face face)
    {
        for (int j = 0; j <teststring.length(); j++)
        {
            ...
            if (ch_glyph_index == 0)
            {
                //CRLog::trace("Required char (%04x)  not found in font %s for lang %s", ch, face->family_name, currlang.c_str());
                return false;
            }
        }
        return true;
...
public:
    ...
    virtual void InitFallbackFonts()
    {
        ...
        {
            faces.addAll(RegisterFont(fonts.at(i), false));
        }
...
//            }
    ...
    LVArray<LanguageWrap> initLangCheckArray()
    {
        ...
        _langCheckArray.add(LanguageWrap(L"in",    L"LAT",L"AZaz09"));
        ...
        _langCheckArray.add(LanguageWrap(L"sr-lat",L"LAT",L"ĐđŽžĆćČčDždžŠš"));

        _langCheckArray.add(LanguageWrap(L"be",    L"CYR",L"ЁёЎў"));
        _langCheckArray.add(LanguageWrap(L"bg",    L"CYR",L"яЯжЖъЪщЩ"));
        _langCheckArray.add(LanguageWrap(L"ru",    L"CYR",L"яЯёЁжЖъЪыЫэЭщЩ"));
        _langCheckArray.add(LanguageWrap(L"uk",    L"CYR",L"IіЇїЄєҐґ"));
        _langCheckArray.add(LanguageWrap(L"sr-cyr",L"CYR",L"ЂђЉљЊњЋћЏџ"));
        ...
        _langCheckArray.add(LanguageWrap(L"hi",    L"OTHER",L"ऄअआइईउऊऋऌऍऎएऐऑऒओऔकखगघङचछजझञटठडढणतथदधनऩपफबभमयरऱलळऴवशषसहऺऻ़ऽािी"));
        _langCheckArray.add(LanguageWrap(L"el",    L"OTHER",L"ΑαΒβΓγΔδΕεΖζΗηΘθΙιΚκΛλΜμΝνΞξΟοΠπΡρΣσΤτΥυΦφΧχΨψΩω"));

        //TODO decide whether he or iw
        _langCheckArray.add(LanguageWrap(L"iw",    L"OTHER",L"אבגדהוזחטיךכלםמןנסעףפץצקרשת"));
...
//            }
    ...
    virtual lString8Collection RegisterFont(lString8 name, bool langcheck)
    {
