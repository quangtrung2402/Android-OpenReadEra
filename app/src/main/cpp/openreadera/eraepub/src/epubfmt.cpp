...
//        }
			...
			if (!href.empty() && !id.empty())
			{
				...
				if (id == coverId)
				{
					...
					if (!stream.isNull())
					{
						...
						{
							CRLog::trace("EPUB coverpage image incorrect! Generating own coverpage!");
				...
				|| mediaType == L"application/x-font-ttf") { // TODO: more media types?
				 ...
				 //LE("fontlist add [%s]",LCSTR(codeBase + href));
				 }
			}
	...
	if (m_doc->getToc()->getChildCount() == 0)
	{
        ...
        if (m_doc->getToc()->getChildCount() == 0)
		{
			CRLog::info("No toc generated, falling back to navdoc");
			CRLog::info("Looking at [%s]",LCSTR(navHref));
			if (!navHref.empty())
			{
...
{
    CRLog::trace("Epub coverpage fallback search attempt...");
    LVStreamRef stream;
    stream = getThumbByAttrValueStrict(dom,container,code_base,lString16("id"),lString16("cover"));
    if(!stream.isNull()){CRLog::trace("Epub coverpage found by id == cover");return stream;}
    stream = getThumbByAttrValueStrict(dom,container,code_base,lString16("properties"),lString16("cover-image"));
    if(!stream.isNull()){CRLog::trace("Epub coverpage found by properties == cover-image");return stream;}
    stream = getThumbByAttrValue(dom,container,code_base,lString16("id"),lString16("/cover"));
    if(!stream.isNull()){CRLog::trace("Epub coverpage found by id containing \"/cover\"");return stream;}
    stream = getThumbByAttrValue(dom,container,code_base,lString16("id"),lString16("cover"));
    if(!stream.isNull()){CRLog::trace("Epub coverpage found by id containing \"cover\"");return stream;}
    stream = getThumbByAttrValue(dom,container,code_base,lString16("id"),lString16("_cover"));
    if(!stream.isNull()){CRLog::trace("Epub coverpage found by id containing \"_cover\"");return stream;}
    stream = getThumbByAttrValue(dom,container,code_base,lString16("href"),lString16("cover"));
    if(!stream.isNull()){CRLog::trace("Epub coverpage found by href containing \"cover\"");return stream;}
    CRLog::error("Epub coverpage fallback search attempt FAILED");
    return stream;
}
