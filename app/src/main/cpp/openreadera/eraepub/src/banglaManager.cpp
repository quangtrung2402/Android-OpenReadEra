...
//
#include "include/banglaManager.h"
#include "ore_log.h"

#include <vector>


BanglaLigMap     gBanglaLigMap;
BanglaLigMapRev  gBanglaLigMapRev;
BanglaFastLigMap gBanglaFastLigMap;
...
dvngLig findBanglaLig(lChar16 ligature)
{
    ...
    BanglaLigMap::iterator it = gBanglaLigMap.find(ligature);
...
lChar16 findBanglaLigRev(dvngLig combo)
{
    ...
    BanglaLigMapRev::iterator it = gBanglaLigMapRev.find(combo);
    if(it==gBanglaLigMapRev.end())
    {
        return 0;
    }
    //LE("findBanglaLigRev return %d", it->second);
    return it->second;
}
...
std::map <dvngLig, lChar16, Comparator> BanglaLigMapReversed()
{
    ...
    for (BanglaLigMap::iterator i = gBanglaLigMap.begin(); i != gBanglaLigMap.end(); ++i)
    {
        lUInt32 fastkey = (i->second.a << 16) + i->second.b;
        if(gBanglaFastLigMap.find(fastkey) == gBanglaFastLigMap.end())
        {
            gBanglaFastLigMap[fastkey] = 1;
        }
        gBanglaLigMapRev[i->second] = i->first;
    }

    //for (auto i = gBanglaFastLigMap.begin(); i != gBanglaFastLigMap.end(); ++i)
    //{
