...
bool LVDocView::LoadDoc(int doc_format, const char *path, uint32_t direct_archive)
{
#ifdef OREDEBUG
...
#endif
    LVStreamRef stream = OreResolveStream(doc_format, path, direct_archive);
    if (!stream)
    {
        return false;
    }
...
{
    lString16 subStr = subStr_in.lowercase();
    if (subStr.length() > in.size() - startPos)
    {
        return -1;
    }
    ...
    for (uint i = startPos; i <= diff_len; i++)
    {
        ...
        for (uint j = 0; j < s_len; j++)
        {
            ...
            lChar16 ch = word.getText().lowercase().ReplaceUnusualSpaces().at(0);
            if ( ch != subStr.at(j))
            {
                flg = 0;
                break;
...
*/
        ...
        void processText(ldomNode *node, ldomXRange *range)
        {
            ...
            for (; start < end; start++)
            {
                ldomWord word = ldomWord(node, start, start + 1);
                list_.add(word);
            }
        }
...
ldomXPointer ldomXRange::getXpointerByOffsetInPageText(int pos)
{
    LVArray<int> pos_arr;
    pos_arr.add(pos);
    return this->getXpointerByOffsetInPageText(pos_arr).get(0);
}
...
LVArray<Hitbox> LVDocView::unionRects(LVArray<Hitbox> rects)
{
    ...
    for (int i = 0; i < rects.size(); i++) {
        ...
        if (gDocumentRTL==1 && rects.get(i).getNode()->isRTL())
        {
        ...
        while (para_counter < para_array.length() && rect.top > para_array.get(para_counter).top)
        {
            ...
            if(para_repeat_counter < PARAEND_REPEAT_MAX && para_rect.top != last_top)
            {
                ...
                if (i >= 1)
                {
                    ...
                    if (gDocumentRTL == 1 && (last_node->isRTL() || node->isRTL()))
                    {
        ...
        if (this->DocToWindowRect(rect))
        {
            if( rect.bottom - rect.top > CHAR_HEIGHT_MIN)
            {
                ...
                if (word_chars.length()>=1)
                {
                    ...
                    if (gDocumentRTL == 1 && last_node->isRTL())
                    {
