...
public:
    ...
    int maxwidth; // width of the widest cell
    bool finished = false;
...
*/
        //TR COLUMNS NEW ALGO
        int colcheck = cols.length();
        for (int i = 0; i < cols.length(); i++)
        {
            int collen = 0;
            for (int j = 0; j < rows.length(); j++)
            {
                CCRTableCell *cell = cells.operator[](j)[i];
                if (cell != NULL && !cell->elem->isNull())
                {
                    collen += cell->elem->getText().length();
                }
            }
            if (collen == 0)
            {
        ...
        {
            int fullWidth = width - TABLE_BORDER_WIDTH * 2;
            LVFont *font = elem->getFont().get();
            for (int i = 0; i < cols.length(); i++)
            {
                CCRTableCol *currcol = cols.get(i);
                if (currcol->txtlen == 0)
                {
                    cols[i]->maxwidth = 5;
                    continue;
                ...
                {
                    CCRTableCell *cell = cells.operator[](j)[i];
                    if (cell == NULL || cell->elem->getText().length() == 0)
                    {
                        continue;
                    }
                    lString16 text = cell->elem->getText();

                    lUInt32 cellwidth = font->getTextWidth((text + "   ").c_str(), text.length() + 3);
                    //CRLog::error("text = [%d][%d][%s]",cellwidth,cellwidth/text.length(),LCSTR(text));
                    if (cellwidth > colmaxwidth)
                    {
                        colmaxwidth = cellwidth;
                    }
                }
                cols[i]->maxwidth = colmaxwidth;
            }
            int rest = 0;
            int unfinished = cols.length();
            for (int i = 0; i < cols.length(); i++)
            {
                cols[i]->width = fullWidth / cols.length();
                if (cols[i]->maxwidth < cols[i]->width)
                {
                    rest += cols[i]->width - cols[i]->maxwidth;
                    cols[i]->width = cols[i]->maxwidth;
                    cols[i]->finished = true;
                    unfinished--;
                }
            }
            if (rest > 0 && unfinished > 0)
            {
                int restdiv = rest / unfinished;
                for (int i = 0; i < cols.length(); i++)
                {
                    if (!cols[i]->finished)
                    {
                        cols[i]->width += restdiv;
                    }
                }
            }
        }
...
LVFontRef getFont(css_style_rec_t * style, int documentId)
{
    ...
    lString8 fontname = style->font_name;
...
int lengthToPx( css_length_t val, int base_px, int base_em )
{
    ...
    case css_val_pt: // 1/72 in
        return (int)(val.value * ( 1 / ((float)72 / (float)DEFAULT_PIXEL_DENSITY)));
    ...
    if ( enode->isElement() )
    {
        ...
        if ( rm==erm_list_item ) {
            ...
            if ( enode->getNodeListMarker( counterValue, marker, marker_width ) ) {
                ...
                for (int i = 1; i < enode->getListLevel(); i++)
                {
                ...
                marker += "\t";
                if(RTL_DISPLAY_ENABLE && gDocumentRTL == 1 && enode->isRTL())
                {
                    margin = 0;
                    marker = "\t" + marker;
                }
                txform->AddSourceLine( marker.c_str(), marker.length(), cl, bgcl, font, flags|LTEXT_FLAG_OWNTEXT, line_h, margin, enode );
...
void DrawDocument( LVDrawBuf & drawbuf, ldomNode * enode, int x0, int y0, int dx, int dy, int doc_x, int doc_y, int page_height, ldomMarkedRangeList * marks,
                   ldomMarkedRangeList *bookmarks, lvRect margins,int columns)
{
    if ( enode->isElement() )
    {
        RenderRectAccessor fmt( enode );
        ...
        case erm_block:
            {
                ...
                {
                    ldomNode * child = enode->getChildNode( i );
                    DrawDocument( drawbuf, child, x0, y0, dx, dy, doc_x, doc_y, page_height, marks, bookmarks,margins,columns ); //+fmt->getX() +fmt->getY()
                }
#if (DEBUG_TREE_DRAW!=0)
                ...
                if ( needBorder ) {
                    ...
                    int currX = doc_x+x0;
                    int currY = doc_y+y0;
                    if(currY < margins.top)    { currY = margins.top; }
                    drawbuf.FillRect( currX,     currY,     currX+w, currY+1,   tableBorderColor );     //top
                    drawbuf.FillRect( currX,     currY,     currX+1, currY+h,   tableBorderColor );     // left
                    drawbuf.FillRect( currX+w-1, currY+2,   currX+w, currY+h+3, tableBorderColorDark ); //right
                    drawbuf.FillRect( currX,     currY+h-1, currX+w, currY+h,   tableBorderColor ); //bottom
                }
            }
            break;
        case erm_list_item:
        ...
        case erm_table_caption:
            {
                ...
                if ( needBorder ) {
                    ...
                    int w = fmt.getWidth();
                    if(currY < margins.top)    { currY = margins.top; }
                    drawbuf.FillRect( currX,     currY+1,   currX+w, currY,   tableBorderColor );
                    drawbuf.FillRect( currX,     currY,     currX+1, currY+h, tableBorderColor );
                    drawbuf.FillRect( currX+w-1, currY,     currX+w, currY+h, tableBorderColorDark );
                    drawbuf.FillRect( currX,     currY+h-1, currX+w, currY+h, tableBorderColorDark );
...
void ApplyInlineStyle(ldomNode * enode, css_style_rec_t* pstyle)
{
    //if (enode->getCrDom()->getDocFlag(DOC_FLAG_EMBEDDED_STYLES) && enode->hasAttribute(LXML_NS_ANY, attr_style)) {
    if (gEmbeddedStylesLVL > 0 && enode->hasAttribute(LXML_NS_ANY, attr_style)) {
        lString16 nodeStyle = enode->getAttributeValue(LXML_NS_ANY, attr_style);
        if (!nodeStyle.empty())
        {
            nodeStyle = cs16("{") + nodeStyle + "}";
            CssStyle Style(nodeStyle);
            //CRLog::error("CssStyle filtered : [%s] -> [%s]",LCSTR(nodeStyle),LCSTR(Style.style_string_));
            nodeStyle = Style.getStyleString();
            LVCssDeclaration decl;
            lString8 s8 = UnicodeToUtf8(nodeStyle);
            const char* s = s8.c_str();
            if (decl.parse(s)) {
                //CRLog::error("style applied to node [%s] = [%s]",LCSTR(enode->getXPath()),LCSTR(nodeStyle));
...
void setNodeStyleRend(ldomNode* enode, css_style_ref_t parent_style, LVFontRef parent_font)
{
    ...
    //inilne style goes second, to have more priority over stylesheet
    //apply inline style
    ApplyInlineStyle(enode,pstyle);
...
#define UPDATE_LEN_FIELD(fld) \
    ...
    switch (pstyle->font_size.type) {
        ...
        case css_val_pt: // 1/72 in
            pstyle->font_size.type = css_val_px;
            pstyle->font_size.value = pstyle->font_size.value * ( 1 / (double)(72 / DEFAULT_PIXEL_DENSITY) );
            break;
        case css_val_unspecified:
        case css_val_in: // 2.54 cm
        case css_val_cm:
