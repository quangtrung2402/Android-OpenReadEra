...
public:
    ...
    lverror_t OpenFile( lString16 fname, lvopen_mode_t mode )
    {
        ...
        if (!file)
        {
        	CRLog::trace( "!opened file: %s mode: %s", UnicodeToLocal(fname).c_str(), modestr);
            m_mode = LVOM_ERROR;
            return LVERR_FAIL;
        }
        m_file = file;
...
//    }
    ...
    lverror_t OpenFile(lString16 fname, int mode) {
    	//CRLog::trace("lverror_t OpenFile: %s", UnicodeToLocal(fname).c_str());
        mode = mode & LVOM_MASK;
#if defined(_WIN32)
        lUInt32 m = 0;
        lUInt32 s = 0;
        ...
        m_fd = -1;
        int flags = (mode == LVOM_READ)
        		? O_RDONLY
        		: O_RDWR | O_CREAT | (use_sync ? O_SYNC : 0) | (mode == LVOM_WRITE ? O_TRUNC : 0);
        lString8 fn8 = UnicodeToUtf8(fname);
        m_fd = open(fn8.c_str(), flags, (mode_t) 0666);
        if (m_fd == -1) {
            CRLog::warn("Error opening file %s for %s, errno=%d, msg=%s",
            		fn8.c_str(),
            		(mode == LVOM_READ) ? "r" : "rw", (int) errno, strerror(errno));
            return LVERR_FAIL;
        }
        struct stat stat;
        if (fstat(m_fd, &stat)) {
...
public:
    ...
    static LVStream* Create(LVStreamRef stream, lvpos_t pos, lString16 name, lUInt32 srcPackSize, lUInt32 srcUnpSize)
    {
        ...
        if(packSize == 0 )
        {
            CRLog::warn("LVZipDecodeStream::Pack size is 0, setting it to unpSize ");
            packSize = unpSize;
        }
...
public:
    virtual LVStreamRef OpenStream( const wchar_t * fname, lvopen_mode_t /*mode*/ )
    {
        ...
        if(found_index<0)
        {
            for (int i = 0; i < m_list.length(); i++)
            {
                if (!lStr_cmp(fname, DecodeHTMLUrlString(lString16(m_list[i]->GetName())).c_str()))
                {
                    if (m_list[i]->IsContainer())
                    {
                        // found directory with same name!!!
...
//                    arcComment=true;
			...
			//removed this part to fix french 00E9 LATIN SMALL LETTER E WITH ACUTE not decoding correctly
            ...
            item->SetItemInfo(lString16(fnbuf), ZipHeader.UnpSize, (ZipHeader.getAttr() & 0x3f));
...
public:
    ...
    */
	...
	lverror_t CreateCopy( LVStreamRef srcStream, lvopen_mode_t mode )
	{
        ...
        if ( (int)sz <= 0 || sz > 0x400000 )
        {
            CRLog::error("CreateCopy error: stream size is not valid!");
            return LVERR_FAIL;
        }
		m_bufsize = sz;
		m_size = 0;
...
lString16 LVCombinePaths( lString16 basePath, lString16 newPath )
{
    ...
    if ( separator == 0 )
        separator = '/';
    if ( newPath.length()>2 && newPath[0]=='.' && newPath[1]==separator )
        newPath.erase(0, 2);
