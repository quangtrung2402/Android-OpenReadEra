...
lString16 ExtractDocGenres(CrDom *dom, lString16 delimiter)
{
    ...
    return genres.ReplaceUnusualSpaces().trim();
}

lString16 ExtractDocAnnotation(CrDom* dom)
{
    return dom->createXPointer(L"/FictionBook/description/title-info/annotation").getText().ReplaceUnusualSpaces().trim();
}
...
{
    lString16 res;
    ldomNode* series = dom->createXPointer(L"/FictionBook/description/title-info/sequence")
            .getNode();
    if (series) {
        ...
        for (int w = 0; w < words.length(); w++)
        {
            ...
            for (int i = 0; i < word.length(); i++)
            {
                ...
                //{
                    ...
                    //NOW PROCESSED IN LvXmlParser::processIndicLigatures();
...
{
    //LE("ApplyEmbeddedStyles() \n[\n%s]",LCSTR(stylesManager.as_string()));
