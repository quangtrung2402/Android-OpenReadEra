...
void CreBridge::processMeta(CmdRequest &request, CmdResponse &response) {
    ...
    LVStreamRef stream = LVDocView::OreResolveStream(format, path, direct_archive);
    if (!stream) {
    ...
    responseAddString(response, lString16::restoreIndicText(title));
    responseAddString(response, lString16::restoreIndicText(authors));
    responseAddString(response, lString16::restoreIndicText(series));
    response.addInt((uint32_t) series_number);
    responseAddString(response, lString16::restoreIndicText(lang));
    responseAddString(response, lString16::restoreIndicText(genre));
    responseAddString(response, lString16::restoreIndicText(annotation));
