...
void processLineToResult_v2(std::vector<Hitbox> &result, fz_context *ctx, fz_rect &bounds, fz_text_line &line)
{
        ...
        if (span->text && span->len > 0)
        {
            for (int textIndex = 0; textIndex < span->len;)
            {
                ...
                else
                {
                    ...
                    if (bbox.x1 < bbox.x0 + (height * 0.1f) || bbox.x1 > bounds.x1)
                    {
                        if(space && isLast)
                        {
                            bbox.x1 = bbox.x0 + (height * 0.2f);
                        }
                        else {
                            bbox.x1 = bbox.x0 + (height * 0.7f);
                        }
    ...
    else if (!space && !hyph)
    {
        ...
        length = fz_runetochar(utf8, text.c);
        charnum++;
        toResult(result, bounds, fz_round_rect(&irect,&rect),utf8,length);
    }
}
