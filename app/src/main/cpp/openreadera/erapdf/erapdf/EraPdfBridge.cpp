...
void MuPdfBridge::processOpen(CmdRequest& request, CmdResponse& response)
{
    ...
    if (!iter.isValid()) {
        LE("Bad request data: %u %u %u %p %p",
                request.cmd, request.dataCount, iter.getCount(), socketName, password);
        response.result = RES_BAD_REQ_DATA;
        return;
    }
...
void MuPdfBridge::processSmartCrop(CmdRequest& request, CmdResponse& response)
{
    ...
    fz_matrix ctm = fz_identity;
    fz_pre_scale(&ctm, (float) smart_crop_w / origin_w, (float) smart_crop_h / origin_h);
    // Manual translating (fz_pre_translate and fz_translate works not as expected)
    ctm.e = -slice_l * (float) smart_crop_w; // Translate left
    ctm.f = -slice_t * (float) smart_crop_h; // Translate top
    ...
    auto pixels = (uint8_t*) malloc(smart_crop_w * smart_crop_h * 4);
    if (renderPage(page_index, smart_crop_w, smart_crop_h, pixels, &ctm)) {
        float smart_crop[4];
...
void MuPdfBridge::processPageRender(CmdRequest& request, CmdResponse& response)
{
    ...
    //LW("RENDER page %d , preview = %d",page_index,preview);
    ctx->erapdf_nightmode = config_invert_images;
    auto pixelsHolder = new CmdData();
    auto pixels = pixelsHolder->newByteArray((w) * (h) * 4);
