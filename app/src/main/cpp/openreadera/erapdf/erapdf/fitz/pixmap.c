...
/*
unsigned int
fz_count_pixmap_luminosity(fz_pixmap *pix)
{
    unsigned char *s = pix->samples;
    int k, x, y;

    unsigned long int hres = 0;
    for (y = 0; y < pix->h; y++)
    {
        unsigned long int wres = 0;
        for (x = 0; x < pix->w; x++)
        {
            unsigned long int pixres = 0;
            for (k = 0; k < pix->n - 1; k++)
            {
                pixres += s[k];
            }
...
RgbColor MakeRgbColor(unsigned char r_, unsigned char g_, unsigned char b_)
{
    RgbColor c;
    c.r = r_;
    c.g = g_;
...
unsigned int
fz_count_pixmap_mode(fz_pixmap *pix)
{
    if(pix->n != 4)
    {
        ERROR_L("MuPdf","pix->n = %d",pix->n);
        return 0;
    }
    ...
    int frequency = pix->h / LINENUM;

    unsigned int gridH = 0;
    unsigned int gridS = 0;
    unsigned int gridV = 0;
    for (int y = 0; y < pix->h; y += frequency)
    {
        int lineH = 0;
        int lineS = 0;
        int lineV = 0;
        ...
        {
            RgbColor rgb = MakeRgbColor(s[0],s[1],s[2]);
...
//    ERROR_L("MuPdf","hsv = [%d:%d:%d]",gridH,gridS*100/255,gridV*100/255);

    frequency = pix->h / ( LINENUM * 2 );
    unsigned char *p = pix->samples;

    long MaxErrors = (pix->w * ( LINENUM * 2 )) * 0.2;
    ...
    for (int y = 0; y < pix->h; y += frequency)
    {
        for (int x = 0; x < pix->w; x++)
        {
            ...
            if (h < gridH - THRESHOLD || h > gridH + THRESHOLD)
            {
                errcounter++;
                if(errcounter > MaxErrors)
                {
                    //ERROR_L("MuPdf","Image is not monoHued , Maxerrors = %ld",MaxErrors)
                    return 0;
                }
            }
            p += pix->n;
        ...
        p+= frequency * pix->w * pix->n;
    }
    //ERROR_L("MuPdf","Image is monoHued, errcounter = %d of %ld",errcounter,MaxErrors)
    return 1;
}
*/

#define ALPHA_THRESHOLD 128 // 50% of alpha

int fz_count_pixmap_needs_invert(fz_context *ctx, fz_pixmap *pix)
{
	...
	int pixnum = 0;
	int alphanum = 0;
	unsigned int coloredpix = 0;
	for (int y = 0; y < pix->h; y += 2)
	{
		for (int x = 0; x < pix->w; x+= 2)
		{
			int pixindex = (y * pix->w * pix->n) + (x*pix->n);

			int curralpha = s[pixindex+pix->n];
			pixnum++;

			int colored = 0;
            for (int i = 0; i < pix->n - 1; i++)
			{
                int currComp = s[pixindex + i];
				int nextComp = s[pixindex + i + 1 ];
				if (!(nextComp >= currComp - 8 && nextComp <= currComp + 8))
				{
					colored = 1;
					break;
				}
			}
			if(curralpha < ALPHA_THRESHOLD)
			{
				alphanum++;
				continue;
			}
			...
			color |= s[pixindex + 0] <<16;
			color |= s[pixindex + 1] <<8;
			color |= s[pixindex + 2] ;


			if  (color >= 0 && color < 0x202020)
			{
				black++;
			}
			else if(color >= 0xC0C0C0 && color <= 0xFFFFFF)
			{
				white++;
			}
        }
	...
	//LE("pixnum = %d",pixnum);
	//LE("coloredpix : %d",coloredpix);
	//LE("black = %d , white = %d , alpha = %d",black,white,alphanum);
	//LE("b+w+a = %d, threshold = %d",black + white + alphanum,pixnum / 100 * 80 );
	int percent = (ctx->previewmode) ? 4 : 2;


	if(coloredpix > (pixnum/100) * percent )
	{
		return 2;
	}
	if(alphanum > (pixnum/100) * 90)
	{
		return -1;
	}
	if ((black + white + alphanum) < (pixnum / 100 * 80))
	{
		return 3;
	}
    return 0;
...
int fz_count_pixmap_needs_invert_for_grayscale_cs(fz_context *ctx, fz_pixmap *pix)
{
    unsigned int black = 0;
    unsigned int white = 0;
    unsigned char *s = pix->samples;
	...
	for (int y = 0; y < pix->h; y += 2)
    {
        ...
        {
	        int pixindex = (y * pix->w * pix->n) + (x*pix->n);
	        int currcolor = s[pixindex];
            int curralpha = s[pixindex+1];
	        pixnum++;
	        if(curralpha < ALPHA_THRESHOLD)
            {
            	alphanum++;
            }
	        else if (currcolor >= 0 && currcolor < 70)
            {
                black++;
            }
            else if(currcolor >= 220 && currcolor <= 255)
            {
                white++;
            }
        }
    ...
    //LE("GCS pixnum = %d",pixnum);
    //LE("GCS black = %d , white = %d",black,white);
    //LE("GCS b+w = %d, threshold = %d",black + white,pixnum / 100 * 80 );
