...
static void
fz_list_fill_image(fz_context *ctx, fz_device *dev, fz_image *image, const fz_matrix *ctm, float alpha, int invert)
{
...
void
fz_run_display_list(fz_context *ctx, fz_display_list *list, fz_device *dev, const fz_matrix *top_ctm, const fz_rect *scissor, fz_cookie *cookie,int pageindex)
{
    int objcounter = 0;
	...
	for (; node != node_end ; node = next_node)
	{
		...
		fz_try(ctx)
		{
			...
			case FZ_CMD_END_PAGE:
				...
				break;
			case FZ_CMD_FILL_PATH:
                    if(ctx->erapdf_nightmode && currobj != NULL)
                    {
	...
	for (int i = page->objcount-1; i >= 0 ; i--)
	{
		...
		for (int j = i+1; j < page->objcount; j++)
		{
            ...
            else
            {
	            ...
	            int contains = fz_contains_point(&curr_rect, c)  ||
	            		       fz_contains_point(&curr_rect, lc) ||
	            		       fz_contains_point(&curr_rect, rc) ||
	            		       fz_contains_point(&curr_rect, tc) ||
	            		       fz_contains_point(&curr_rect, bc) ||
	            		       fz_contains_point(&curr_rect, lt) ||
	            		       fz_contains_point(&curr_rect, rt) ||
	            		       fz_contains_point(&curr_rect, lb) ||
	            		       fz_contains_point(&curr_rect, rb);
	            if (!contains)
	            {
		            continue;
	            }
			...
			switch (next->type)
			{
	 			case FZ_CMD_FILL_IMAGE : imgcount++ ; if( next->invert > 0) {imgsurface_inv += surface;} else {imgsurface += surface;} break;
				case FZ_CMD_FILL_TEXT  : txtcount++ ; if( next->invert > 0) {txtsurface_inv += surface;} else {txtsurface += surface;} break;
				case FZ_CMD_FILL_PATH  : pthcount++ ; if( next->invert > 0) {pthsurface_inv += surface;} else {pthsurface += surface;} break;
				default:
					break;
			}
...
void
fz_run_fake_display_list(fz_context *ctx, fz_display_list *list, fz_device *dev, const fz_matrix *top_ctm, const fz_rect *scissor, fz_cookie *cookie, int pageindex)
{
