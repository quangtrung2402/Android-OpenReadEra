	...
	for (i = 0; i < text->len; i++)
	{
		...
		if (glyph)
		{
			...
			else
			{
                ...
                fz_paint_image(state->dest, &state->scissor, state->shape, pixmap, &mat, alpha * 255, ctx->flag_interpolate_images);
			}
...
static void
fz_draw_fill_image(fz_context *ctx, fz_device *devp, fz_image *image, const fz_matrix *ctm, float alpha, int invert)
{
	fz_draw_device *dev = (fz_draw_device*)devp;
	...
	fz_try(ctx)
	{
        ...
        if (ctx->erapdf_nightmode)
        {
            if (invert == 1)
            {
                ...
                fz_paint_image(state->dest, &state->scissor, state->shape, inverted, &local_ctm, alpha * 255, ctx->flag_interpolate_images);
                inverted_flag = 1;
            }
            else
            {
                fz_paint_image(state->dest, &state->scissor, state->shape, pixmap, &local_ctm, alpha * 255, ctx->flag_interpolate_images);
            }
        }
        else
        {
	...
	{
		// EraPDF: inverted mode >>>
		if (ctx->erapdf_nightmode && inverted_flag) {
	...
	fz_colorspace *colorspace, float *color, float alpha)
{
	...
	fz_try(ctx)
	{
		...
		if (ctx->erapdf_nightmode) {
			...
			memcpy(inverted->samples, pixmap->samples, pixmap->n * pixmap->w * pixmap->h);
			fz_invert_pixmap(ctx, inverted);
			fz_paint_image_with_color(state->dest, &state->scissor, state->shape, inverted, &local_ctm, colorbv, ctx->flag_interpolate_images);
		} else {
			fz_paint_image_with_color(state->dest, &state->scissor, state->shape,   pixmap, &local_ctm, colorbv, ctx->flag_interpolate_images);
		}
		// EraPDF: inverted mode <<<
...
fz_draw_clip_image_mask(fz_context *ctx, fz_device *devp, fz_image *image, const fz_rect *rect, const fz_matrix *ctm)
{
	...
	fz_try(ctx)
	{
        ...
        fz_paint_image(mask, &bbox, state->shape, pixmap, &local_ctm, 255, ctx->flag_interpolate_images);
	}
	fz_always(ctx)
	{
		fz_drop_pixmap(ctx, scaled);
