...
{
	// EraPDF: open special filename as file descriptor >>>
	static const char fdspec[] = "/fd:";
	GUTF8String urlname = url.UTF8Filename();
	const char* url_ptr = urlname;
	if (!GStringRep::cmp(fdspec, url_ptr, sizeof(fdspec)-1)) {
	    url_ptr += sizeof(fdspec)-1;
	    int fd = dup(atoi(url_ptr));
	    return fd;
	}
	// EraPDF: open special filename as file descriptor <<<
