...
ddjvu_page_t* DjvuBridge::getPage(uint32_t pageNo, bool decode)
{
    ...
    if (decode)
    {
        ...
        if (r != DDJVU_JOB_OK)
        {
            LE("Cannot decode page: %d %d", pageNo, r);
        }
    }
    return pages[pageNo];
}
...
void DjvuBridge::handleMessages()
{
    ...
    while ((msg = ddjvu_message_peek(context)))
    {
        ...
        {
        case DDJVU_ERROR:
            LE("Djvu decoding error: %s %s %d", msg->m_error.filename, msg->m_error.function, msg->m_error.lineno);
            break;
        case DDJVU_INFO:
            break;
        case DDJVU_DOCINFO:
